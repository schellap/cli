...
{
    auto process_coreclr = [&package_dir, &package_cache_dir, &svc = m_svc]
        (bool is_portable, const pal::string_t& deps_dir, deps_json_t* deps) -> pal::string_t
    {
        ...
        if (deps->has_coreclr_entry())
        {
            ...
            if (entry.is_serviceable && svc.find_redirection(entry.library_name, entry.library_version, entry.relative_path, &candidate))
            {
